...
impl<'a, T: 'a> RingBuffer<'a, T> {
    ...
    pub fn enqueue_one_with<'b, R, F, E>(&'b mut self, f: F) -> Option<Result<R, E>>
        ...
        let result = f(&mut self.storage[index]);
    ...
    pub fn enqueue_one<'b>(&'b mut self) -> Option<&'b mut T> {
        self.enqueue_one_with(Ok)
            .map(|result| result.unwrap_or_else(|x| x))
    }
    ...
    pub fn dequeue_one(&mut self) -> Option<&mut T> {
...
impl<'a, T: 'a> From<Slice<'a, T>> for RingBuffer<'a, T> {
    fn from(slice: Slice<'a, T>) -> RingBuffer<'a, T> {
